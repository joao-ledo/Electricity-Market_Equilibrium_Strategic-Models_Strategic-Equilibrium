...
SDE_Producer.. sum(y, sum(u $ UnitGeneratorConect(y, u), o(y, u)*p(y, u))) + sum(k, sum(u $ UnitGeneratorConect(k, u), o_aux(k, u)*p(k, u))) - sum(j, sum(c $ UnitConsumerConect(j, c), b_aux(j, c)*d(j, c))) =e= - sum(i, sum(u $ UnitGeneratorConect(i, u), mu_p_max(i, u)*p_max(i, u))) - sum(j, sum(c $ UnitConsumerConect(j, c), mu_d_max(j, c)*d_max(j, c)));
SDE_Consumer.. sum(i, sum(u $ UnitGeneratorConect(i, u), o_aux(i, u)*p(i, u))) - sum(z, sum(c $ UnitConsumerConect(z, c), b(z, c)*d(z, c))) - sum(l, sum(c $ UnitConsumerConect(l, c), b_aux(l, c)*d(l, c))) =e= - sum(i, sum(u $ UnitGeneratorConect(i, u), mu_p_max(i, u)*p_max(i, u))) - sum(j, sum(c $ UnitConsumerConect(j, c), mu_d_max(j, c)*d_max(j, c)));
...
Model MPEC_StrategicProducer_PC_DC_SDE /Objective_Function_Producer, Strategic_Offering_Boundary, balance, producer_lowerboundary, producer_upperboundary,
consumer_lowerboundary, consumer_upperboundary, Deriv_p_S, Deriv_p_NS, Deriv_d_NS, SDE_Producer,
Dual_Positivity_mu_p_min, Dual_Positivity_mu_p_max, Dual_Positivity_mu_d_min, Dual_Positivity_mu_d_max/;

Model MPEC_StrategicConsumer_PC_DC_SDE /Objective_Function_Consumer, Strategic_Bidding_Boundary, balance, producer_lowerboundary, producer_upperboundary,
consumer_lowerboundary, consumer_upperboundary, Deriv_p_NS, Deriv_d_S, Deriv_d_NS, SDE_Consumer,
Dual_Positivity_mu_p_min, Dual_Positivity_mu_p_max, Dual_Positivity_mu_d_min, Dual_Positivity_mu_d_max/;
...
Z('J1') = no;
L('J1') = yes;
solve MPEC_StrategicProducer_PC_DC_SDE maximizing OF_Producer using NLP;
...
o_previus(i, u) $ UnitGeneratorConect(i, u) = o_aux(i, u);
o_aux(y, u) $ UnitGeneratorConect(y, u) = o.l(y, u);
******************************************************************************************************************************************************************************
...
Z('J1') = yes;
L('J1') = no;
solve MPEC_StrategicConsumer_PC_DC_SDE maximizing OF_Consumer using NLP;
...
b_previus(j, c) $ UnitConsumerConect(j, c) = b_aux(j, c);
b_aux(z, c) $ UnitConsumerConect(z, c) = b.l(z, c);
****************************************************************************************************************************************************************************
...
count = 1;
* This loop structure runs in a particular set, in this case set kk
loop(kk $ ((sum(i, sum(u $ UnitGeneratorConect(i, u), abs(o_previus(i, u) - o_aux(i, u)))) ge 0.1) or  (sum(j, sum(c $ UnitConsumerConect(j, c), abs(b_previus(j, c) - b_aux(j, c)))) ge 0.1)),
